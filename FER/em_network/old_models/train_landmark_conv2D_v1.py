...
# set seed, make result reproducable
SEED = 1234
random.seed(SEED)
np.random.seed(SEED)
...
def train(model, data_loader, criterion, optimizer, epoch=0, to_log=None, print_freq=25):
    ...
    for i, (lm_data, target) in enumerate(data_loader):
        ...
        lm_data = torch.permute(lm_data, (0, 3, 2, 1))
...
if __name__ == "__main__":
    ...
    # log path
    path = dir_path("landmark_conv2d_xy_v1", result_dir)

    # create model
    model = LandmarkNet_2D(subnet=SubNet_2D, classifier=Classifier, num_channel=config['num_channels'],
