    ...
    for i, conc_data in enumerate(data_loader):
        ...
        if i % print_freq == 0:
            str = ('Epoch: [{0}][{1}/{2}]\t'
                ...
                epoch, i, len(data_loader), batch_time=batch_time, cls_losses=cls_losses, kd_losses=0,
...
if __name__ == "__main__":
    ...
    path = dir_path("evaluate_KD", result_dir)
    ...
    s_checkpoint = os.path.join(result_dir, "Supervision_SUM_image2D_KD_20220629-152906", 'model_best.pth.tar')
    ...
    sc_checkpoint = os.path.join(result_dir, "Supervision_SUM_image2D_KD_20220629-152906", 'classifier_best.pth.tar')
    assert os.path.exists(sc_checkpoint), 'Error: no checkpoint directory found!'
    student_classifier.load_state_dict(torch.load(sc_checkpoint))
    student_classifier = student_classifier.to(device)

    tf_block = TFblock_v4()
    tf_checkpoint = os.path.join(result_dir, "Supervision_SUM_image2D_KD_20220629-152906", 'block_best.pth.tar')
    assert os.path.exists(sc_checkpoint), 'Error: no checkpoint directory found!'
