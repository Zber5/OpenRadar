...
from models.Conv2D import ImageSingle_Student, ImageSingle_v1, ImageDualNet_Single_v1, ImageNet_Large_v1, \
    Classifier_Transformer, ImageDualNet_Frames, ImageNet_Large_Frames
...
def train(teacher_model, student_model, data_loader, criterion, optimizer, epoch=0, to_log=None, print_freq=25):
    ...
    for i, conc_data in enumerate(data_loader):
        ...
        t_fmap = t4
        s_fmap = s4
        s_fmap = torch.mean(s_fmap, dim=1)
        s_fmap = tf_block(s_fmap)
        kd_loss = pair_loss_old(s_fmap, t_fmap)
...
def test(s_model, t_model, test_loader, criterion, to_log=None):
    ...
    with torch.no_grad():
        ...
        format_str = 'Test set: Average loss: {:.4f}, Average KD loss: {:.4f}, Accuracy: {}/{} ({:.2f}%)\n'.format(
            test_loss, kd_test_loss, correct, len(test_loader.sampler), acc)
        print(format_str)
        if to_log is not None:
...
if __name__ == "__main__":
    ...
    path = dir_path("Supervision_image2D_Transformer_cumulatedSum_L3_sum_LargeTFblock", result_dir)
    ...
    # heatmap datasets
    heatmap_train = HeatmapDataset(heatmap_root, h_train_ann, cumulated=False, frame_cumulated=True,
                                   num_frames=config['h_num_frames'])
    heatmap_test = HeatmapDataset(heatmap_root, h_test_ann, cumulated=False, frame_cumulated=True,
                                  num_frames=config['h_num_frames'])
    ...
    # student model
    student_model = ImageSingle_v1(num_classes=config['num_classes'], block=ImageDualNet_Frames,
                                   subblock=ImageNet_Large_Frames)
    ...
    optimizer = torch.optim.Adam(
        list(student_model.parameters()) + list(tf_block.parameters()) + list(extractor.parameters()) + list(
            student_classifier.parameters()), lr=config['lr'])
