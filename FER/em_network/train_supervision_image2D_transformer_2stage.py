...
if __name__ == "__main__":
    ...
    path = dir_path("Supervision_image2D_Transformer_stage2_featureBP_updated", result_dir)
    ...
    if config['stage'] == 's2':
        s_checkpoint = os.path.join(result_dir, "Supervision_image2D_Transformer_stage1_updated_20220216-094921",
                                    'model_best.pth.tar')
    ...
    if config['stage'] == 's2':
        tf_checkpoint = os.path.join(result_dir, "Supervision_image2D_Transformer_stage1_updated_20220216-094921",
                                     'block_best.pth.tar')
    ...
    if config['stage'] == 's1':
        optimizer = torch.optim.Adam(
            list(student_model.parameters()) + list(tf_block.parameters()), lr=config['lr'])
    else:
        # optimizer = torch.optim.Adam(
