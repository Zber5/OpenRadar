...
def train(teacher_model, student_model, data_loader, criterion, optimizer, epoch=0, to_log=None, print_freq=25):
    ...
    for i, conc_data in enumerate(data_loader):
        ...
        loss = cls_loss + kd_loss * 2

        train_loss.append(loss.item())
        dist_loss_list.append(kd_loss.item())
        loss.backward()
        optimizer.step()
    ...
    return train_loss, dist_loss_list
...
def test(s_model, t_model, test_loader, criterion, to_log=None):
    ...
    with torch.no_grad():
        ...
        return test_loss.item(), acc, dist_loss_list
...
if __name__ == "__main__":
    ...
    path = dir_path("Supervision_SUM_image2D_Baseline3", result_dir)
    # path = dir_path("Supervision_SUM_image2D_TransformerLarge_L3_Baseline3_S8", result_dir)
    ...
    train_loader = DataLoader(dataset_train, num_workers=1, pin_memory=False, batch_size=config['batch_size'])
    test_loader = DataLoader(dataset_test, num_workers=1, pin_memory=False, batch_size=config['batch_size'])
    ...
    for epoch in range(config['num_epochs']):
        train_loss, dist_loss = train(teacher_model, student_model, data_loader=train_loader, criterion=criterions,
                                        optimizer=optimizer, epoch=epoch, to_log=path['log'])
        test_loss, acc, test_dist_loss = test(student_model, teacher_model, test_loader=test_loader, criterion=criterion_test,
                                                to_log=path['log'])
        if acc >= best_acc:
        ...
        metrics_dic['train_loss'].append(np.mean(train_loss))
        metrics_dic['train_distance_loss'].append(np.mean(dist_loss))
        metrics_dic['test_distance_loss'].append(np.mean(test_dist_loss))
