...
if __name__ == "__main__":
    ...
    for epoch in range(config['num_epochs']):
        ...
        if acc >= best_acc:
            ...
            save_checkpoint(tf_block.state_dict(), is_best=True, checkpoint=path['dir'], name="block_best.pth.tar")
            save_checkpoint(student_classifier.state_dict(), is_best=True, checkpoint=path['dir'],
                            name="classifier_best.pth.tar")
        else:
            save_checkpoint(student_model.state_dict(), is_best=False, checkpoint=path['dir'], epoch=epoch)
            save_checkpoint(tf_block.state_dict(), is_best=False, checkpoint=path['dir'], name="block_{}.pth.tar",
                            epoch=epoch)
            save_checkpoint(student_model.state_dict(), is_best=False, checkpoint=path['dir'],
                            name="classifier_{}.pth.tar", epoch=epoch)
