...
from models.c3d import C3D_VIDEO_out, C3DFusionBaseline_out
from models.c3d_autoencoder import AutoencoderWithClassifier_v2, Embedding_LOSS
import pandas as pd
...
def train(teacher_model, student_model, data_loader, criterion, optimizer, epoch=0, to_log=None, print_freq=25):
    ...
    teacher_model.eval()
    student_model.autoencoder.eval()
    ...
    for i, conc_data in enumerate(data_loader):
        ...
        # t_out1, pseudo_targets = teacher_model(v_inputs)
        _, outputs = student_model(azi, ele)

        # normalize embeddings
        # t_out1 = embedding_fun(t_out1)
        # s_out1 = embedding_fun(s_out1)

        # loss = criterion_kd(s_out1, t_out1, torch.ones(t_out1.size(0)).to(device))
        loss = criterion_cls(outputs, targets)
        # loss = cls_loss + kd_loss * config['lambda_kd']
...
if __name__ == "__main__":
    ...
    # create model
    teacher_model = C3D_VIDEO_out(sample_size=config['imag_size'], sample_duration=config['v_num_frames'],
    ...
    student_model = AutoencoderWithClassifier_v2(sample_duration=config['h_num_frames'],
                                                 num_classes=config['num_classes'])

    s_checkpoint = os.path.join(result_dir, "C3D_TF_autoencoder_v2_20211221-081250", 'last.pth.tar')
