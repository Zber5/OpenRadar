...
from models.c3d_autoencoder import Autoencoder, RankingLossFunc, AutoencoderWithClassifier
import pandas as pd
...
def train(teacher_model, student_model, data_loader, criterions, optimizer, epoch=0, to_log=None, print_freq=25):
    ...
    for i, conc_data in enumerate(data_loader):
        ...
        t_out, _ = teacher_model(v_inputs)
        s_out, outputs = student_model(azi, ele)

        kd_loss = criterion_kd(s_out, t_out)
        cls_loss = criterion_cls(outputs, targets)
        loss = (cls_loss + kd_loss * config['lambda_kd']) / 2
...
if __name__ == "__main__":
    ...
    # create model
    teacher_model = C3D_VIDEO_V2(sample_size=config['imag_size'], sample_duration=config['v_num_frames'],
                                 num_classes=config['num_classes'])
    checkpoint = os.path.join(result_dir, "C3D_Video_V2_20211211-120742", 'best.pth.tar')
    ...
    student_model = AutoencoderWithClassifier(sample_duration=config['h_num_frames'], num_classes=config['num_classes'])
    student_model = student_model.to(device)

    # initialize critierions
    # criterion_kd = RankingLossFunc(config['delta'])
    criterion_kd = NST()
    ...
    for epoch in range(config['num_epochs']):
        train_loss = train(teacher_model, student_model, data_loader=train_loader, criterions=criterions,
                           optimizer=optimizer, epoch=epoch,
                           to_log=path['log'])
        test_loss, acc = test(student_model, test_loader=test_loader, criterion=criterion_cls, to_log=path['log'])
        if acc >= best_acc:
            best_acc = acc
