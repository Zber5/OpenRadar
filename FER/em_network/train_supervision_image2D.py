...
from models.Conv2D import ImageSingle_Student
import pandas as pd
...
def train(teacher_model, student_model, data_loader, criterion, optimizer, epoch=0, to_log=None, print_freq=25):
    ...
    for i, conc_data in enumerate(data_loader):
        ...
        kd_loss = criterion_kd(s_out1, t_out1)
        # cls_loss = criterion_cls(outputs, targets)
        cls_loss = criterion_cls(outputs, pseudo_targets, targets)
...
if __name__ == "__main__":
    ...
    # teacher model
    checkpoint = os.path.join(result_dir, "Pretrained_ResNet_video_20220115-184648", 'best.pth.tar')
    ...
    # student model
    student_model = ImageSingle_Student(num_classes=config['num_classes'])
