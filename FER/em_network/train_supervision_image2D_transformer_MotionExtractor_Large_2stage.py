...
def train(teacher_model, student_model, data_loader, criterion, optimizer, epoch=0, to_log=None, print_freq=25):
    ...
    for i, conc_data in enumerate(data_loader):
        ...
        # kd_loss = pair_loss_v1(s_fmap, t_fmap)
...
def test(s_model, t_model, test_loader, criterion, to_log=None):
    ...
    with torch.no_grad():
        for i, conc_data in enumerate(test_loader):
            ...
            s_fmap = s4
            s_fmap = tf_block(s_fmap)
            # loss = pair_loss_v1(s_fmap, t_fmap)
            loss = pair_loss(s_fmap, t_fmap)
            s_fmap = avgpool(s_fmap)
            s_input = s_fmap.view((s_fmap.size(0), -1))
...
class TFblock_v2(nn.Module):
    def __init__(self, dim_in=512, dim_inter=128):
...
if __name__ == "__main__":
    ...
    path = dir_path("Supervision_image2D_Transformer_extractor_large_MaxMean_stage1_pairLossV1", result_dir)

    best_folder = "Supervision_image2D_Transformer_extractor_large_MaxMean_stage1_pairLossV1_20220225-163244"
    ...
    tf_block = TFblock_v3(dim_in=512, dim_inter=1024)
    if config['stage'] == 's2':
