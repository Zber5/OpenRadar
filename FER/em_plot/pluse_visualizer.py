...
# multi virtual antenna plot setting
antenna_order = [i for i in range(0, 4)]
...
def plot_phase_change_in_one(range_data, bin_index=0, is_diff=True, loop_index=0):
    fig, axes = plt.subplots(1, 1, figsize=(12, 5))

    # fig1, axes1 = plt.subplots(1, 1, figsize=(12, 5))
    # v_order = [8, 10, 7, 9, 6, 4, 5, 3]

    # numOfFrame , numOfChirp, numOfRX
    ...
    for o, color in zip(antenna_order, tab_color):
        ...
        # original phase differences plot
        ax.plot(va_diff_phase, linewidth=2, c=color, label='Virtual Antenna {}'.format(va_index), zorder=5)

        # filtered phase differences plot
        ax.plot(va_diff_phase_filtered, linewidth=1.5, linestyle='dashed', c=color, label='Virtual Antenna Filtered{}'.format(va_index), zorder=6)
...
def plot_phase_map(range_data, num_bins=None, v_antenna=None, loop_index=None, is_phase=False, is_diff=True):
    ...
    if is_phase and is_diff:
        ...
        img = ax.imshow(np.abs(sig_phase.T), cmap='bwr', interpolation='none', aspect='auto')
...
if __name__ == '__main__':
    ...
    # range_data = arange_tx(range_data, num_tx=numTxAntennas)
    # plot phase map
    # plot_phase_map(range_data, num_bins=30, v_antenna=5, loop_index=5, is_phase=True, is_diff=False)
