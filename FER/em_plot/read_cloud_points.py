...
configFileName = 'C:/Users/Zber/Documents/Dev_program/OpenRadar/FER/profile/xwr18xx_profile_cloud_points.cfg'
...
def update():
    ...
    if dataOk and len(detObj["x"]) > 0:
        # print(detObj)
        for xx, yy, zz in zip(-detObj["x"], detObj["y"], detObj["z"]):
            all_data.append(np.asarray([frameNumber, xx, yy, zz]))
        x = -detObj["x"]
        y = detObj["y"]
        s.setData(x, y)
...
while True:
    ...
    # Stop the program and close everything if Ctrl + c is pressed
        ...
        np.save('clouds_points', np.asarray(np.stack(all_data, axis=1)))
        CLIport.close()
        Dataport.close()
        win.close()
        break
