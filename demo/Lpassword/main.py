...
if __name__ == '__main__':
    ...
    adc_data = adc_data[:, :, 1, :numDisplaySamples]
    ...
    # chirp selection
    select_range_bin = select_range_bin[:,3::3]
    ...
    x1 = butter_highpass_fs(sig=unwrap, lowcut=20, fs=50, output='sos')
...
#         print("Using Custom Plotting")
    ...
    # plot doppler
    plt.imshow(doppler_data[:, :, bin_index].T)
    # plt.plot(np.std(doppler_data[:,:, bin_index], axis=1))
    plt.show()
    ...
    plt.plot(doppler_var)
    plt.plot(peaks, doppler_var[peaks], "x", s=100)
    plt.show()

    # plot snr
    plt.imshow(snr_data[:, :, bin_index].T)
    plt.show()

    # plos snr line
    # var = np.std(snr_data[:, :, 6], axis=0)
    # bin_index = np.argmax(var, axis=0)
    # plt.plot(snr_data[:, bin_index, 6])
    # plt.show()
