...
if __name__ == '__main__':
    ...
    if loadData:
        ...
        # adc_data = np.fromfile('C:/ti/mySavedData/eyeblink_f220_1_Raw_0.bin', dtype=np.int16)
        # adc_data = np.fromfile('C:/ti/mySavedData/lipmotion_F220_1.bin', dtype=np.int16)
        # adc_data = np.fromfile('C:/ti/mySavedData/lipmotion_data.bin', dtype=np.int16)
        adc_data = np.fromfile('C:/ti/mySavedData/eye_blinking_1_Raw_0.bin', dtype=np.int16)
    ...
    var = np.std(adc_data, axis=(0,1))
    # #
    bin_index = np.argmax(var[:50], axis=0)
    plt.bar(np.arange(0,numADCSamples)[:50],var[:50])
    plt.show()
